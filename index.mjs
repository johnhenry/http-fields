...
const StructuredFields = (() => {
    ...
    const parseDisplayString = (input) => {
        if (input.length < 2 || input[0] !== '%' || input[1] !== '"') {
            throw new Error('Expected "%" followed by quote at start of display string');
        }
        input.shift(); // consume '%'
        input.shift(); // consume opening quote
    ...
    const serializeList = (list) => {
        return list.map(member => {
            const serializedValue = Array.isArray(member.value) 
                ...
                : serializeBareItem(member.value) + serializeParameters(member.parameters || {});
            return serializedValue;
        }).join(', ');
    };

    const serializeInnerList = (innerList, listParams = {}) => {
        const items = innerList.map(item => 
            serializeBareItem(item.value) + serializeParameters(item.parameters || {})
        ).join(' ');
    ...
    const serializeDictionary = (dict) => {
        return Object.entries(dict).map(([key, member]) => {
            if (member.value === true) {
                ...
                return key + '=' + serializedValue + serializeParameters(member.parameters || {});
            }
        }).join(', ');
    };
    ...
    return {
            ...
            switch (fieldType) {
                ...
                case 'item':
                    if (typeof data !== 'object' || data === null) {
                        throw new Error('Item data must be an object with value and parameters');
                    }
         ...
         */
        displayString: (unicodeValue) => ({ type: 'displaystring', value: unicodeValue })
    };
})();
