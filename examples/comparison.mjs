...
console.log('# Side-by-Side Comparison: Our Implementation vs @badgateway/structured-headers\n');
...
console.log('const result = StructuredFields.parse(\'' + headerValue + '\', \'dictionary\');');
console.log('```\n');

const ourResult = StructuredFields.parse(headerValue, 'dictionary');
...
console.log('  accept: {');
console.log('    value: StructuredFields.token("application/json"),');
console.log('    parameters: { q: 0.9 }');
console.log('  },');
console.log('  cache: { value: true, parameters: {} }');
console.log('};');
console.log('const header = StructuredFields.serialize(data, "dictionary");');
...
console.log('  token: { value: StructuredFields.token("app"), parameters: {} },');
console.log('  binary: { value: StructuredFields.binary("SGVsbG8="), parameters: {} },');
...
console.log('```javascript');
console.log('try {');
console.log('  const result = StructuredFields.parse("invalid[", "list");');
...
console.log('    return value ? StructuredFields.parse(value, type) : null;');
console.log('  }');
console.log('  ');
...
console.log('**@badgateway Implementation:**');
console.log('```javascript');
console.log('import { parseDictionary, serializeDictionary } from "structured-headers";');
console.log('');
console.log('class StructuredHeaders {');
console.log('  constructor(headers = new Headers()) {');
console.log('    this.headers = headers;');
...
console.log('  theme: { value: StructuredFields.token("dark"), parameters: {} },');
console.log('  notifications: { value: true, parameters: { email: true } }');
console.log('};');
console.log('document.cookie = `prefs=${StructuredFields.serialize(cookieData, "dictionary")}`;');
console.log('');
console.log('// Advanced use case examples');
console.log('const iotDevice = {');
console.log('  "device-type": { value: StructuredFields.token("thermostat"), parameters: {} },');
console.log('  "battery-level": { value: 0.73, parameters: { "low-power": false } }');
console.log('};');
console.log('```\n');
...
console.log('- **@badgateway:** ~25KB minified (includes TypeScript overhead)\n');
...
console.log('- **Our Implementation:** Simpler mental model, extensive examples');
...
console.log('  for (const [key, {value, parameters}] of Object.entries(ourResult)) {');
...
console.log('Both libraries are excellent - the choice depends on your specific needs! 🎉');
