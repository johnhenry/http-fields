...
const examples = {
  ...
  token: { value: HTTPFields.token("app"), parameters: {} },
...
```javascript
try {
  const result = HTTPFields.parse("invalid[", "list");
} catch (error) {
...
class StructuredHeaders {
  ...
  getStructured(name, type) {
    ...
    return value ? HTTPFields.parse(value, type) : null;
  }
...
const cookieValue =
  "theme=dark, language=en-US, timezone=America/New_York, notifications;email;push=?0;frequency=daily";
const preferences = HTTPFields.parse(cookieValue, "dictionary");
```
...
function parseStructuredCookies(req, res, next) {
  ...
  Object.entries(structuredCookieMap).forEach(([name, type]) => {
    if (req.cookies[name]) {
      try {
        req.structuredCookies[name] = HTTPFields.parse(req.cookies[name], type);
      } catch (error) {
...
authorization: Bearer token123
x-client-metadata: web-app;version="1.0.0";user-agent="HTTPFieldsDemo/1.0"
```
