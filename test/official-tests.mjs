      ...
      if (Array.isArray(item) && item.length === 2) {
        ...
        return {
          value: innerList.map((innerItem) => {
            ...
            return {
              value: convertValue(innerValue),
              parameters: convertParams(innerParams),
            };
          }),
          parameters: convertParams(params),
        };
  ...
  if (Array.isArray(normalizedValue)) {
    normalizedValue = normalizedValue.map((innerItem) => {
      if (innerItem && typeof innerItem === "object" && "value" in innerItem) {
        ...
        if (normalizedInnerValue && typeof normalizedInnerValue === "object") {
          if (normalizedInnerValue.type === "binary") {
            normalizedInnerValue = {
              type: "binary",
              value: base64ToBase32(normalizedInnerValue.value),
            };
          } else if (normalizedInnerValue.type === "token") {
            normalizedInnerValue = {
              __type: "token",
              value: normalizedInnerValue.value,
            };
        ...
        const params = Object.entries(innerItem.parameters || {}).map(
          ([key, value]) => {
            // Convert parameter values to official test format
            const convertedValue =
              value && typeof value === "object" && value.type === "binary"
    ...
    for (const testCase of tests) {
      ...
      test(testName, () => {
        ...
        if (mustFail) {
          // Test should fail
          assert.throws(() => {
            HTTPFields.parse(raw, headerType);
          }, `Expected "${raw}" to fail parsing but it didn't`);
        } else {
          // Test should succeed
          try {
            const result = HTTPFields.parse(raw, headerType);
            ...
            if (
              ...
              JSON.stringify(convertedExpected)
            ) {
              console.log("\nTest:", testName);
              console.log("Raw:", raw);
              console.log(
                "Expected:",
                JSON.stringify(convertedExpected, null, 2)
              );
