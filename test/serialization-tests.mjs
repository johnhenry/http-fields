...
function loadSerializationTests(filename) {
    ...
    try {
        ...
        console.warn(`Could not load serialization test file ${filename}:`, error.message);
        return [];
    }
}
                ...
                if (testCase.expected) {
                    ...
                    if (headerType === 'item') {
                        if (Array.isArray(testCase.expected)) {
                            ...
                            dataToSerialize = { value: convertValue(value), parameters: convertParams(params) };
                        } else {
                            dataToSerialize = { value: convertValue(testCase.expected), parameters: {} };
                        }
                        ...
                        for (const [key, valueWithParams] of Object.entries(testCase.expected)) {
                            if (Array.isArray(valueWithParams)) {
                                const [value, params] = valueWithParams;
                                dataToSerialize[key] = { value: convertValue(value), parameters: convertParams(params) };
                            } else {
                                dataToSerialize[key] = { value: convertValue(valueWithParams), parameters: {} };
                            }
                        }
                    }
                    ...
                    if (mustFail) {
                        ...
                        assert.throws(() => {
                            StructuredFields.serialize(dataToSerialize, headerType);
                        }, `Expected serialization to fail but it didn't`);
                    } else {
                        // Test should succeed in serialization
                        try {
                            const serialized = StructuredFields.serialize(dataToSerialize, headerType);
                            assert.ok(serialized, 'Serialization should produce valid result');
                        } catch (error) {
                    ...
                    const raw = Array.isArray(testCase.raw) ? testCase.raw.join(', ') : testCase.raw;
                    if (mustFail) {
                        assert.throws(() => {
                            const parsed = StructuredFields.parse(raw, headerType);
                            StructuredFields.serialize(parsed, headerType);
                        }, `Expected "${raw}" to fail serialization but it didn't`);
                    } else {
                        try {
                            const parsed = StructuredFields.parse(raw, headerType);
                            const serialized = StructuredFields.serialize(parsed, headerType);
                            const reparsed = StructuredFields.parse(serialized, headerType);
                            assert.ok(reparsed, 'Round-trip serialization should produce valid result');
                        } catch (error) {
                            throw new Error(`Failed serialization round-trip for "${raw}": ${error.message}`);
                        }
                    }
                }
            });
